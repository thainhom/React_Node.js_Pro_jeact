...
function UserForm({ userId, onSubmit, onCancel }) {
    ...
    const validate = () => {
        ...
        if (user.username.length < 4 || user.username.length > 10 || /^[A-Za-z0-9]$/.test(user.username)) {
        ...
        } else if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(user.email.match)) {
        ...
        if (confirmationPassword !== password) {
            errors.set('confirmation_password', 'Xác nhận mật khẩu không trùng khớp.');
        }
